...
############## Task 12
# Set the feature of all no des to the same value
#features = np.eye(n) # Generates node features
features = np.ones((n,n))
#We see here that the accuracy is worse when we set all the node features to the same value rather than when we set them to the identity matrix. This can be explained by the fact that when e have an identity matrix all the node are considred as different at the beginning and then the model can learn to group them together and predict a class. However, when we set all the node features to the same value, the model cannot distinguish between the nodes and thus cannot learn the class of each node.

...
for epoch in range(epochs):
    ...
    for i in range(0, N_train, batch_size):
        ...
        for j in range(i, min(i+batch_size, N_train)):
            adj_batch.append(nx.adjacency_matrix(G_train[j])+sp.identity(G_train[j].number_of_nodes()))
            idx_batch.extend([j-i]*G_train[j].number_of_nodes())
            y_batch.append(y_train[j])
        adj_batch = sp.block_diag(adj_batch).toarray()
        features_batch = np.ones((adj_batch.shape[0], 1))

        adj_batch = sparse_mx_to_torch_sparse_tensor(sp.coo_matrix(adj_batch)).to(device)
...
for i in range(0, N_test, batch_size):
    ...
    for j in range(i, min(i+batch_size, N_test)):
        adj_batch.append(nx.adjacency_matrix(G_test[j])+sp.identity(G_test[j].number_of_nodes()))
        idx_batch.extend([j-i]*G_test[j].number_of_nodes())
